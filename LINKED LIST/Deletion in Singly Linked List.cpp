...
{
    // deleting first node i.e. if pos==0
    if(pos==0)
    {
        Node*temp = head;
        head = head->next;
        temp->next = NULL;         // NOT NECESSARY             // doing this before deleting pointer current to avoid SEGMENTATION FAULT
        delete temp;
    }
    // deleting node inside linked list
    else{
        Node*current = head;
        Node*previous = NULL;
        int count=1;
        ...
        // after coming out of this while, current is standing at the node we want to delete
        previous->next = current->next;
        current->next = NULL;             // NOT NECESSARY    // doing this before deleting pointer current to avoid SEGMENTATION FAULT
        delete current;
